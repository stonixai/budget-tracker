...
export function NextThemeToggle({ variant = 'simple', className = '' }: NextThemeToggleProps) {
  const { theme, setTheme, resolvedTheme } = useTheme();
  const [mounted, setMounted] = useState(false);
  const [isOpen, setIsOpen] = useState(false);

  // Avoid hydration mismatch - wait for client-side mount
  useEffect(() => {
    setMounted(true);
  }, []);
  ...
  if (!mounted) {
    return (
      <div className={`w-9 h-9 rounded-lg bg-gray-200 dark:bg-gray-800 animate-pulse ${className}`} />
    );
  }
